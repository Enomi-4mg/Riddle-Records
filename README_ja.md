...
- Gallery の作品データは `src/content/gallery/*.md` で管理します。
- `src/data/gallery.ts` は legacy 互換用に残していますが、現在の `galleryItems` は空です。
- Jekyll 時代のファイルは移行後に削除済みです。

## Works と Gallery ページ

`/works/` は作品全体の総合一覧です。`src/content/gallery/*.md` の Gallery 作品と `src/content/songs/` の楽曲をまとめ、共通の Works カードとして表示します。

`/gallery/` はイラスト・3DCG・写真などに絞った作品アーカイブとして残しています。Gallery 個別ページは `/gallery/[slug]/` に生成されますが、対象は `detail: true` を指定した item のみです。

Gallery frontmatter の主なフィールド:

- `slug`: Gallery 個別ページの URL に使う文字列
- `detail`: `true` の場合のみ `/gallery/[slug]/` を生成
- `title`: 作品タイトル
- `date`: 作品日付。`YYYY-MM-DD` 形式で書きます
- `image`: Cloudinary の public ID または画像URL
- `description`: カード、meta、リード文に使う短い説明
- Markdown 本文: 個別ページ本文。省略時は `description` を本文にも使います
- `tags`: Works / Gallery / 個別ページに表示するタグ
- `article_url`: 関連する Journal 記事、または既存の作品記事
- `making_article_url`: 関連するメイキング記事
- `thumbnail`: `true` の場合、Journal 一覧などのサムネイル照合候補に含めます。`false` で除外します
- `draft`: `true` の場合、production build から除外します

legacy フィールド:

- `cloudinary_id`: 旧データ互換として helper 側で読みます。新規Markdownでは `image` を使います
- `categories`: 旧データ互換として helper 側で読みます。新規Markdownでは `tags` を使います
...
- Works カードで個別ページがない場合は、`article_url`、フル画像の順にリンク先を決めます。
- Header の active 判定では、`/gallery/` は引き続き Works グループとして扱います。

## Content Editor での作成・編集手順

Journal / Songs / Gallery / Projects は、独立アプリ `journal-editor-app/` の **Riddle Records Content Editor** で Markdown を作成・編集します。フォルダ名は今回は変更せず、実体として Content Editor として運用します。

旧Astro版Editor `src/pages/tools/journal-editor.astro` は、現在は移行案内ページです。旧URL `/tools/journal-editor/` はブックマークや Information ページからの互換用に残していますが、content 編集の本命Editorは `journal-editor-app/` です。画像カード生成やGallery登録用コード生成も新Editorへ移植済みです。

### 1. エディタを開く

Content Editorを起動します。
...
公開サイト上では `Information` ページの「Content Editor（移行案内）」から、移行先と起動方法を確認できます。

### 2. 記事設定を入力する

新Editorでは、選択した content kind に合わせて frontmatter を生成します。Editor UI の入力補助は `journal-editor-app/src/types/content.ts`、Astro 側の schema は `src/content/config.ts` が担当します。
...
dev server 上では `src/content/<kind>/*.md` を直接管理します。UI は note 的な入力体験のままですが、正本はブラウザの `localStorage` ではなく Markdown ファイルです。build/公開環境ではローカルファイル書き込みができないため、出力欄のコピーまたは `.md` ダウンロードで運用します。
...
作品をGalleryにも載せる場合は、記事作成後に `src/content/gallery/*.md` へ作品Markdownを追加・編集します。
