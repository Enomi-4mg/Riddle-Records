...
1. **記事ファイルを作成**
   - 以下の命名規則に従ってファイル名を設定します。
     - **通常記事:** `YYYY-MM-DD.md`
     - **メイキング記事:** `YYYY-MM-DD-[作品名]-making.md`
   - 例: `_journal/2026-01-17.md` または `_journal/2025-06-06-five-apples-making.md`
   - ファイル名の年月日とフロントマターの `date` を一致させる必要があります。
...
### ✍️ Journal記事でのLaTeX利用

MathJax を利用して、Journal記事にLaTeX構文で数式を埋め込むことができます。

1.  **フロントマターでMathJaxを有効化**
    - 記事でLaTeXを使用するには、マークダウンファイルのフロントマターに `use_math: true` を追加します。
...
2.  **LaTeXコードを記述**
    - **インライン数式**: 数式を1つのドル記号 (`$`) で囲みます。
      - 例: `これはインライン数式です: $E = mc^2$`
    - **ブロック数式**: 数式を2つのドル記号 (`$$`) で囲みます。
      - 例:
        ```
        これはブロック数式です:
        $$
...
この機能は `_layouts/post.html` で設定されており、`page.use_math` が `true` の場合にのみMathJaxスクリプトを読み込みます。
