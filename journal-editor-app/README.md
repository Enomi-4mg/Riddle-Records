...
dev server 上では、Editor から `src/content/journal/*.md` を直接開いて編集できます。DraftList の `journal files` からファイルを選ぶと実ファイルのMarkdownを読み込み、Editor画面の `Markdown保存` で同じ `.md` ファイルへ書き戻します。

この直接保存はローカルの Vite dev server API に依存します。build後の公開環境ではローカルファイル書き込みはできないため、従来通り `出力` からコピーするか `.md` ボタンでMarkdownを書き出してください。

API は `src/content/journal/` 配下のサブディレクトリなし `.md` ファイルだけを対象にします。絶対パス、path traversal、`.md` 以外の拡張子、journal ディレクトリ外への書き込みは拒否します。
...
5. `Markdown保存` で `src/content/journal/` に保存します。build/公開環境では `出力` からコピー、または `.md` ボタンでMarkdownを書き出します。
...
1. dev server 上では `Markdown保存` で `src/content/journal/` に直接保存します。
2. build/公開環境では Editor の `.md` でMarkdownを書き出し、推奨ファイル名に従って `src/content/journal/` に配置します。
